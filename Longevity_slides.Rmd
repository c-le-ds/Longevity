...
use complete.cases function and run it on the pilling/timmers/cases90/cases99 input file we downloaded
  which column has lots of missings. 
try formatted file and lift to build 37, use liftover package

- do loo analysis and check if one of the non-european SNPs has an outlier effect on estimate
  - include it in the discussion section
  - grep, how many exposures have at least one SNP from a non-European population
    -careful use of mrbase. 
    - for any exposures we want to examine, make sure SNPs from non-Euro study doesn't have undo influence
