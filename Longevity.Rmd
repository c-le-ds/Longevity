...
x<-unique(exposures_gwas$id.exposure)
...
# to keep only gwas results, use code below
# #remove eqtl, met-a,b,c and prot-a,b,c
# exposures_gwas <- exposures_gwas %>%
#   filter(!grepl('eqtl-a', id.exposure))%>%
#   filter(!grepl('met-a', id.exposure))%>%
#   filter(!grepl('met-b', id.exposure))%>%
#   filter(!grepl('met-c', id.exposure))%>%
#   filter(!grepl('prot-a', id.exposure))%>%
#   filter(!grepl('prot-b', id.exposure))%>%
#   filter(!grepl('prot-c', id.exposure))
...
saveRDS(error_log, "data/harmonized/error_log_timmers_gwas_ieu.rds")


saveRDS(timmers, "data/harmonized/timmers_gwas_ieu_ls.rds")
saveRDS(timmers_df, "data/harmonized/timmers_gwas_ieu_df.rds")
```
...
```{r}
gwas <- readRDS("data/harmonized/pilling_df_gwas_ieu.rds")
mr_gwas <- gwas %>% 
  filter(!grepl('ieu-a-819', id.exposure))%>% # ieu-819 exposure.beta = 0, causes error in MR analysis. pull out
  filter(error == "FALSE") %>%
  mr()%>%
...
saveRDS(mr_gwas, "data/MR/MR_pilling_gwas_ieu.rds")
```
...
saveRDS(MR, "data/MR/Pilling_multtest_gwas_ieu.rds")
...
```{r}
gwas <- readRDS("data/harmonized/timmers_gwas_ieu_df.rds")
mr_gwas <- gwas %>%
  filter(!grepl('ieu-a-819', id.exposure))%>% # ieu-819 exposure.beta = 0, causes error in MR analysis. pull out
  filter(error == "FALSE") %>%
  mr()%>%
...
saveRDS(mr_gwas, "data/MR/MR_timmers_gwas_ieu.rds")
```
...
saveRDS(MR, "data/MR/Timmers_multtest_gwas_ieu.rds")
```
### III. Cases 90 from Deleen et al
Deleen cases 90

```{r}
gwas <- readRDS("data/harmonized/cases_90_gwas_ieu_df.rds")
mr_gwas <- gwas %>%
  filter(!grepl('ieu-a-819', id.exposure))%>% # ieu-819 exposure.beta = 0, causes error in MR analysis. pull out
  filter(error == "FALSE") %>%
  mr()%>%
...
# mr <- rbind(mr_gwas, mr_protein, mr_metabolite)
# 
 saveRDS(mr_gwas, "data/MR/MR_cases_90_gwas_ieu.rds")
```
...
saveRDS(MR, "data/MR/Cases90_multtest_gwas_ieu.rds")
```

### IV. Cases 99 from Deleen et al
```{r}
gwas <- readRDS("data/harmonized/cases_99_gwas_ieu_df.rds")
...
# mr <- rbind(mr_gwas, mr_protein, mr_metabolite)
 ...
 saveRDS(mr_gwas, "data/MR/MR_cases_99_gwas_ieu.rds")
```
...
saveRDS(MR, "data/MR/Cases99_multtest_gwas_ieu.rds")
```
