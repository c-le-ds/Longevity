...
#ex
outcome_pilling_gwas <- read_outcome_data(
    snps = exposures_gwas$SNP,
...
try to obtain from gwas catalog

```{r}
pilling_out_gwas <- extract_outcome_data(snps = exposures_gwas$SNP, outcomes = 'ebi-a-GCST006698')
```


Dataframes are not the same length

using data downloaded onto local drive we get 4993 SNPs with 12 variables for the outcome

using id from link we get 51655 SNPS and proxies with 23 variables


Comparing the two we see the outcome data generated from the downloaded data, is within the larger dataset.

```{r}


comparedf(outcome_pilling_gwas, pilling_out_gwas) %>% summary()
...
#### c. UK biobank 90 cases

SNPs must match in both exposure and outcome data

```{r}

#downloaded data

#take only the SNPs that are in the outcome data
rsid <- outcome_pilling_gwas$SNP

#deal with duplicates
#keep row with lowest p-value
#keep row with highest Beta

expo_pilling_gwas_download <- data.frame()

for(i in 1:4993){
  
  rsid_unique<- exposures_gwas %>%
    filter(SNP == rsid[i])
  
  if(tally(rsid_unique) > 1) {
    value <-  min(rsid_unique$pval.exposure)
    
    rsid.keep <- rsid_unique %>%
    filter(pval.exposure == value)
  }
  ...
  if(tally(rsid.keep) >1) {
    beta_max <- max(rsid.keep$beta.exposure, na.rm = T)
    
    if(beta_max == -Inf){
      rsid.keep <- rsid.keep[1,]
    }
    else{
      rsid.keep <- rsid.keep %>%
      filter(beta.exposure == beta_max)
    }
  ...
  expo_pilling_gwas_download <- rbind(expo_pilling_gwas_download, rsid.keep)
...
#retrieved data
#take only the SNPs that are in the outcome data
rsid <- pilling_out_gwas$SNP

#deal with duplicates
#keep row with lowest p-value
#keep row with highest Beta

exp_pilling_gwas <- data.frame()
...
for(i in 1:length(rsid)){
  ...
  exp_pilling_gwas <- rbind(exp_pilling_gwas, rsid.keep)
...
pilling_download <-  harmonise_data(
    expo_pilling_gwas_download,
    outcome_pilling_gwas
)


#retrieved data

pilling_out_gwas <- pilling_out_gwas[1:16]
