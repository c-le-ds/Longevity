...
saveRDS(mr, "data/MR/MR_timmers.rds")
```
...
mr_gwas <- mr(gwas)%>%
  mutate(exposure_dat = "GWAS")
saveRDS(mr_gwas, "data/harmonized/MR_cases_90_gwas.rds")
...
saveRDS(MR, "data/MR/Cases90_multtest.rds")
```

#### d. Cases 99 from Deleen et al
```{r}
gwas <- readRDS("data/harmonized/cases_99_gwas_df.rds")

mr_gwas <- mr(gwas)%>%
  mutate(exposure_dat = "GWAS")
saveRDS(mr_gwas, "data/harmonized/MR_cases_99_gwas.rds")
protein <-readRDS("data/harmonized/cases_99_protein_df.rds")
mr_protein <- mr(protein) %>%
  mutate(exposure_dat = "protein")

metabolite <- readRDS("data/harmonized/cases_99_metabolites_df.rds")
mr_metabolite <- mr(metabolite)  %>%
  mutate(exposure_dat = "metabolite")
...
saveRDS(mr, "data/MR/MR_cases_99.rds")
```
...
saveRDS(MR, "data/MR/Cases99_multtest.rds")
```
