...
# to make things run faster, the outcome and harmonizing steps will be split


# For each outcome create a list of databases
# as a function



cases_99p <- foreach(i = 72001:72005, .packages = c("tidyverse", "TwoSampleMR")) %dopar% {
...
stopCluster(c1) 
cases_99_genes <- c(cases_99_genes, cases_99p)

cases_99_df <- bind_rows(cases_99_genes)
 


saveRDS(cases_99_genes, "../data/harmonized/cases_99_genes.rds")
saveRDS(cases_99_df, "../data/harmonized/cases_99_genes_df.rds")
...
```{r}
 exp_unique <- exposures_genes[!duplicated(exposures_genes$exposure),] %>% pull(exposure)
harmonize <- function(i, exp_unique, file){
  
cases_99p<- vector(mode = "list", length = length(exp_unique))
...
run it
```{r}
cases_99 <- lapply(215001:225000,harmonize, exp_unique, file = "C:/Users/Me/Desktop/Longevity/data/outcomes/cases99rsid.csv" )
cases_99_genes <- c(cases_99_genes,cases_99)
...
Harmonize and combine mapped Pilling data
...
saveRDS(error_log, "../data/harmonized/error_log_timmers_genes.rds")


saveRDS(timmers, "../data/harmonized/timmers_genes_ls.rds")
saveRDS(timmers_df, "../data/harmonized/timmers_genes_df.rds")
```




## Test doParallel for larger exposures
...
x <- rnorm(100)
 ...
 squareandroot <- function(root = NULL, square = NULL, half = NULL, double = NULL)
{
  dat <- list(
    root = root,
    square = square
  ...
  #set class name
  class(dat) <- append(class(dat), "squareandroot")
  return(dat)
  
}
 y <- foreach(i = 1:length(x)) %dopar% {
   root <- sqrt(x[i])
   square <- (x[i])^2
   
   result <- squareandroot()
   result$root <- root
